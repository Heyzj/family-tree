  ...
  return (
    <div id="top" className="w-full">
      ...
      {data.origins && (
        <section
          id="origins"
          className="py-24 px-20 scroll-mt-28 bg-gradient-to-br from-blue-50 via-white to-blue-100"
        >
          <ScrollReveal>
      ...
      {data.figures && (
        ...
        >
          ...
          {data.figures?.items && data.figures.items.length > 0 && (
            <div className="grid grid-cols-3 gap-10 mt-16 items-stretch">
              {data.figures?.items?.slice(0, 3).map((person, idx) => (
                <ScrollReveal key={idx} delayMs={idx * 90} className="h-full">
                  <div className="bg-white/80 backdrop-blur-md border border-blue-200 rounded-xl overflow-hidden shadow-lg hover:shadow-xl group hover:-translate-y-2 transition-all duration-300 h-full flex flex-col">
                    ...
                    <div className="p-8 flex flex-col flex-grow">
                      ...
                      <Link
                        ...
                        className="inline-flex items-center gap-1 px-4 py-2 text-sm font-medium text-blue-600 bg-blue-50/60 backdrop-blur-md border border-blue-200 rounded-lg hover:bg-blue-100/80 hover:shadow-md transition-all self-start"
                      >
                        了解更多
                      </Link>
                    </div>
      ...
      {data.events && (
        ...
        >
            ...
            <div className="grid grid-cols-2 gap-10">
              {data.events?.items?.slice(0, 2).map((event, idx) => (
                <ScrollReveal key={idx} delayMs={idx * 100}>
                  <Link
                    to={`/event/${encodeURIComponent(event?.name || `event-${idx}`)}`}
          ...
          {(data.events?.moreCta?.label || data.events?.moreLink?.label) && (
            <ScrollReveal delayMs={250}>
              <div className="text-center mt-16">
                <Link
                  to="/events"
                  className="inline-block bg-white/60 backdrop-blur-md border border-blue-200 text-blue-600 px-8 py-3 rounded-xl font-medium hover:bg-white/80 hover:shadow-lg transition-all"
                >
                  {data.events?.moreCta?.label ||
                    data.events?.moreLink?.label}
                </Link>
              </div>
            </ScrollReveal>
          )}
